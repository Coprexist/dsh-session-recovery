...
export function repairFile(file, { raw, dryRun = false, out, compact = false } = {}) {
  const report = []
  ...
  for (const w of wire) {
    ...
    for (const b of blocks) {
      if (b && typeof b.text === 'string') totalChars += b.text.length
      if (b && typeof b.arguments === 'string') totalChars += b.arguments.length
    }
  }
