...
export function repairFile(file, { raw, dryRun = false, out, compact = false, maxTokens } = {}) {
  ...
  // ---- max-tokens fallback: if the (possibly compacted) transcript is still
  // over budget, drop the OLDEST surface messages until it fits. Guarantees a
  // resumed session can always build a request (context-length overflow guard).
  let trimmedOldest = 0
  if (maxTokens && Number.isFinite(maxTokens) && maxTokens > 0) {
    const budgetChars = maxTokens * 1.0 // ~1 char/token worst case (CJK); code/English are cheaper
    let total = 0
    const perEvent = new Map()
    for (const ev of workEvents) {
      if (isSurface(ev.type)) {
        const n = surfaceCharCount(ev)
        perEvent.set(ev.seq, n)
        total += n
      }
    }
    if (total > budgetChars) {
      ...
      for (const ev of workEvents) {
        if (!isSurface(ev.type)) continue
        if (acc <= budgetChars) break
        drop.add(ev.seq)
        acc -= perEvent.get(ev.seq) ?? 0
      }
      // never trim the newest surface message (the user's latest input)
      for (let i = workEvents.length - 1; i >= 0; i--) {
        if (isSurface(workEvents[i].type)) { drop.delete(workEvents[i].seq); break }
      }
      if (drop.size) {
        workEvents = workEvents.filter(ev => !drop.has(ev.seq))
        trimmedOldest = drop.size
        report.push(`max-tokens fallback: trimmed ${trimmedOldest} oldest messages to fit ${maxTokens} tokens (est ${budgetChars} chars)`)
      }
    }
  }
