  ...
  if (ctx.commands) {
    disposers.push(ctx.commands.register({
      ...
      handler: async (invocation) => {
        ...
        const mtIdx = tokens.indexOf('--max-tokens')
        let maxTokens
        let rest = tokens
        if (mtIdx >= 0) {
          const v = tokens[mtIdx + 1]
          if (v !== undefined) { maxTokens = Number(v); rest = tokens.filter((_, i) => i !== mtIdx && i !== mtIdx + 1) }
          else rest = tokens.filter((_, i) => i !== mtIdx)
        }
        const target = rest.filter(t => !t.startsWith('--')).pop() || invocation.agent.session.id
        try {
          const file = resolveTarget(target, dshHome())
          if (!file) {
            return {
              kind: 'error',
              text: `找不到会话文件：${target}\n已搜索 ${path.join(dshHome(), 'sessions')}\n用法：/session-repair [--dry-run] [--apply] [--compact] [--max-tokens N] [会话id或路径]`,
            }
          }
          const r = repairFile(file, { dryRun, compact, maxTokens })
